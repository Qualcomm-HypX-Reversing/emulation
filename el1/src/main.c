...
const extern unsigned long _heap;
...
int main(){
    ...
    uh_call(UH_APP_INIT, 2, EL1_LOAD, 0x210000, 1, 0); //voffset and physaddr start are probably the same
  //  uh_call(UH_APP_INIT, 2, EL1_LOAD+0x1000, 0x4000000+0x1000, 1, 0); //voffset and physaddr start are probably the same


    unsigned long param_1 = (unsigned long)&_heap;
    unsigned long param_2 = 0x200000;
